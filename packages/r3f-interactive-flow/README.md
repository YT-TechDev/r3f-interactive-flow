...
A single `FlowProvider` can wrap DOM controls, input helpers, progress UI, and a Canvas subtree. DOM navigation and `useFlowProgress` do not require Canvas, and a remounted Canvas can observe the current provider-owned flow again through read-only `useFlowFrame` consumers. Input hooks call the existing `next` and `prev` controls. Accepted input navigation may call `preventDefault()` when enabled; rejected navigation remains unprevented. `useWheelInput` and `useTouchInput` skip `preventDefault()` for explicitly non-cancelable events without making navigation acceptance depend on event cancelability. `preventDefault: true` does not guarantee that native browser behavior can always be interrupted. Reduced motion is application-owned: pass the transition timing your app wants, and use a React `key` remount only when you intentionally want to apply a new provider configuration and reset flow state.
