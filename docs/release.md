...
- It installs with `pnpm install --frozen-lockfile`, runs `pnpm audit --prod`, runs the full safe `pnpm release:check`, and then publishes through Changesets with npm provenance.
...
Trusted Publishing requires GitHub Actions OIDC. Public-package provenance should be generated by the trusted publishing path. Actual OIDC publishing cannot be validated from a pull request because it requires the external npm Trusted Publisher settings and an intentional release dispatch.
